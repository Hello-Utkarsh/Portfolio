...
const theme: ThemeConfig = {
  light: { patternbgColor: "#DCDFE2", bgColor: "#D9DCE0", titleTxt: "#484E5B", secondaryText: "#6C7789", secondaryClr: "#7B62F9", socialIcnBg: "#A0A8B1", socialIcnClr: "#212121", techIcn: "black", typography },
  dark: { patternbgColor: "black", bgColor: "#101010", titleTxt: "white", secondaryText: "#99a1af", secondaryClr: "#ACA0E6", socialIcnBg: "#212121", socialIcnClr: "#D2D6DA", techIcn: "white", typography },
}
...
export default function page() {
  ...
  const [page, setPage] = useState<'home' | 'project' | 'blog'>('home')
      ...
      <motion.div
        ...
        <div className="grid grid-cols-5 gap-2 lg:gap-3 row-span-3">
          <div className="grid col-span-4 grid-rows-2 gap-2 lg:gap-3">
            <div className="grid grid-cols-4 gap-3">
              <div className="col-span-2 rounded-xl border-2 border-[#212121] flex flex-col justify-center items-center text-start px-6 py-2">
                <p className={cn(theme[selected].typography.heading, "w-full")} style={{ color: theme[selected].titleTxt }}>Hola Amigo</p>
                <p className={cn(theme[selected].typography.descPara, "mt-1 w-full")} style={{ color: theme[selected].secondaryText }}>I'm a full-stack developer building backend systems, developer tools, and AI-powered applications while exploring scalable software architecture.</p>
              </div>
              <div className="col-span-1 rounded-xl border-2 border-[#212121] flex justify-center items-center">
                <span className="flex flex-col items-center">
                  ...
                  <p className={cn(theme[selected].typography.heading, "mt-1")} style={{ color: theme[selected].titleTxt }}>Past Works</p>
                </span>
              </div>
            ...
            <div className="grid grid-cols-4 gap-3">
              <div className="col-span-1 rounded-xl grid grid-cols-2 border-2 px-2 py-2 gap-2 border-[#212121]">
                <Link href={"https://github.com/Hello-Utkarsh"} target="_blank" className="h-full w-full">
                  <span className="flex cursor-pointer justify-center items-center rounded-xl h-full w-full" style={{ backgroundColor: theme[selected].socialIcnBg }}>
                    <GithubIcon fillColor={theme[selected].socialIcnClr} />
                  </span>
                </Link>
                <Link href={"https://mail.google.com/mail/?view=cm&fs=1&to=pateluttkarsh697@gmail.com"} target="_blank" className="h-full w-full">
                  <span className="flex cursor-pointer justify-center items-center rounded-xl h-full w-full" style={{ backgroundColor: theme[selected].socialIcnBg }}>
                    <EmailIcon fillColor={theme[selected].socialIcnClr} />
                  </span>
                </Link>
                <Link href={"https://github.com/Hello-Utkarsh"} target="_blank" className="h-full w-full">
                  <span className="flex cursor-pointer justify-center items-center rounded-xl h-full w-full" style={{ backgroundColor: theme[selected].socialIcnBg }}>
                    <LinkedinIcon fillColor={theme[selected].socialIcnClr} />
                  </span>
                </Link>
                <Link href={"https://github.com/Hello-Utkarsh"} target="_blank" className="h-full w-full">
                  <span className="flex cursor-pointer justify-center items-center rounded-xl h-full w-full" style={{ backgroundColor: theme[selected].socialIcnBg }}>
                    <TwitterIcon fillColor={theme[selected].socialIcnClr} />
                  </span>
                </Link>
              </div>
              <div className="col-span-1 rounded-xl border-2 border-[#212121] flex flex-col justify-start md:py-2 md:px-2 lg:py-3">
                ...
                <div className="grid grid-cols-5 gap-2 lg:mt-2">
                  ...
                  <NodeIcon />
                  <ReactIcon />
                  <NextIcon fillColor={theme[selected].techIcn} />
                  <PythonIcon />
                  <GitIcon />
                  <PostgresIcon fillColor={theme[selected].techIcn} />
                  <PrismaIcon fillColor={theme[selected].techIcn} />
                  <TailwindIcon />
                  <TypescriptIcon />
                  <JavascriptIcon />
                </div>
              ...
              <div className="col-span-1 rounded-xl border-2 border-[#212121] flex flex-col justify-center items-center gap-2 lg:gap-3">
                ...
                <p className={cn(theme[selected].typography.heading, "leading-5")} style={{ color: theme[selected].titleTxt }}>More of<br />My Work</p>
                <button onClick={() => setPage('project')} className="text-black text-sm px-2 py-1 lg:text-base lg:px-4 kg:py-1 rounded-xl cursor-pointer" style={{ backgroundColor: theme[selected].secondaryClr }}>View More</button>
              </div>
            </div>
          </div>
          <div className="col-span-1 grid grid-rows-5 gap-3">
            <div className="row-span-2 rounded-xl border-2 border-[#212121] overflow-x-scroll flex justify-center items-center px-2">
              <GitHubCalendarSafe selected={selected} />
            </div>
        ...
        <div className="grid grid-cols-5 gap-3 row-span-2">
            ...
            <div className="flex px-5 gap-6 h-full w-full">
              <div className="w-fit flex flex-col justify-between">
                <span className="">
                  ...
                  <p className={cn(theme[selected].typography.heading)} style={{ color: theme[selected].titleTxt }}>Blogs</p>
                </span>
          ...
          <div className="col-span-2 rounded-xl border-2 border-[#212121] px-5 py-4 flex flex-col justify-center">
            ...
            <p className={cn(theme[selected].typography.heading, "mt-4")} style={{ color: theme[selected].titleTxt }}>Let's Work Together</p>
            <p className={cn(theme[selected].typography.descPara, "mt-1")} style={{ color: theme[selected].secondaryText }}>Lets create something unique together! I'm excited to collaborate</p>
            <button className="bg-[#212121] w-fit rounded-full p-1 lg:p-2 mt-2 -rotate-35 self-end flex hover:scale-125 transition hover:-rotate-45 cursor-pointer"><ArrowRight /></button>
          </div>
        </div>
      </motion.div>)
    }
...
const SliderToggle = ({ selected, setSelected }: any) => {
  return (
    <div className="relative flex w-fit items-center gap-2 rounded-full">
      ...
      >
        <FiMoon className="relative size-5 lg:size-6 z-10 text-lg md:text-sm" />
      </button>
      <button
      ...
      >
        <FiSun className="relative size-5 lg:size-6 z-10 text-lg md:text-sm" />
      </button>
      <div
